...
class VGGPipeline(BasePipeline):
    ...
    def setup_loaders(self) -> None:
        ...
        if os.path.exists(weights_cache_path):
            self.rebalancing_weights = torch.load(weights_cache_path, map_location=self.device)
        ...
        if 0 < subset_percent < 1.0:
            ...
            print(f"VGG Pipeline - Using {num_samples_train} samples ({subset_percent*100:.1f}%)")
    ...
    def train_epoch(self, epoch_num: int) -> float:
        ...
        progress_bar = tqdm(self.train_loader, desc=f"VGG Epoch {epoch_num}", leave=False)
        for lll_inputs, ab_targets_continuous in progress_bar:
            ...
            per_pixel_loss = torch.abs(ab_pred - ab_targets_continuous).sum(dim=1)  # [B, H, W]
    ...
    def evaluate(self, visualize: bool = False) -> float:
        ...
        with torch.no_grad():
            for lll_inputs, ab_targets_continuous in tqdm(self.val_loader, desc="VGG Eval", leave=False):
                lll_inputs = lll_inputs.to(self.device)
                ab_targets_continuous = ab_targets_continuous.to(self.device)
    ...
    def visualize_batch(self, lll_inputs, ab_targets_continuous, ab_pred):
        ...
        for i in range(min(lll_inputs.size(0), 5)):
            ...
            ground_truth_color_image = ColorizationUtils.reconstruct_image(l_input, ground_truth_ab)
            predicted_color_image = ColorizationUtils.reconstruct_image(l_input, predicted_ab)
    ...
    def run_training(self) -> None:
        ...
        visualization_epoch_interval = self.config["training"].get("visualization_epoch_interval", 50)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            should_visualize = (
                epoch == 1 or 
                epoch % visualization_epoch_interval == 0 or 
                epoch == num_epochs
            )
            ...
            if val_loss < best_val_loss:
                ...
                best_model_path = os.path.join(self.best_model_dir, "best_model_rebal.pth")
            ...
            if self.early_stopping(val_loss, self.model):
                ...
                if self.early_stopping.best_state:
                    ...
                    best_early_model_path = os.path.join(self.best_model_dir, "best_model_earlystop_rebal.pth")
